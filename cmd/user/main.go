...
import (
	...
	"github.com/Ajulll22/belajar-microservice/internal/user/router"
...
func main() {
	err := godotenv.Load("./config/global.env", "./config/user.env")
	if err != nil {
		log.Println(err)
	}
	cfg := config.GetEnv()
	...
	port := fmt.Sprintf(":%s", cfg.USER_SERVICE_PORT)
	r.Run(port)
}
